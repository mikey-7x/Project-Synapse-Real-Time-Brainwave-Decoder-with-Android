...
MODEL_KNN = "knn_model.joblib"
MODEL_SVM = "svm_model.joblib"
MODEL_LR  = "lr_model.joblib"
MODEL_XGB = "xgb_model.joblib"
...
def train_models(path, fs):
    ...
    # === Classical Models ===
    RandomForestClassifier(n_estimators=300).fit(X_feat, y)
    dump(RandomForestClassifier(n_estimators=300).fit(X_feat, y), MODEL_RF)
    dump(KNeighborsClassifier(n_neighbors=3).fit(X_feat, y), MODEL_KNN)
    dump(LogisticRegression(max_iter=1000).fit(X_feat, y), MODEL_LR)
    dump(SVC(probability=True).fit(X_feat, y), MODEL_SVM)
    dump(XGBClassifier(use_label_encoder=False, eval_metric='logloss').fit(X_feat, y), MODEL_XGB)
    print("[✓] RF, KNN, LR, SVM, XGB models trained.")
...
def predict_realtime():
    ...
    if model_type == "lstm":
        ...
        if model_type in model_map:
            model = joblib_load(model_map[model_type][0])
...
def main():
    ...
    while True:
        ...
        if c == "1":
            ...
            model_type = input("Enter model (rf, knn, lr, svm, xgb, lstm): ").strip().lower()
